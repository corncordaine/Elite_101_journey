...
if(highest_point > 0 && candles[i] > highest_point){
    printf("\n--- BOS ALERT! Price %.2f broke above previous high %.2f ---", candles[i], highest_point);
     highest_point = candles[i]; 
    
}
if(swing_high(candles,i,size)){
    highest_point = candles[i];
    printf("\nnew resistance of %.2f found", highest_point);
}
